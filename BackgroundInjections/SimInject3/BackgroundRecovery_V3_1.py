...
#Logrithmically sampled frequencies instead of linearly
freqs = np.logspace(np.log10(1/Tspan), np.log10(30/Tspan),30)

###################################
#From libstempo to pick same freqs to search over as injected
...
dt = dur/npts
howm1 = 10
freqs = np.arange(0, 1/(2*dt), 1/(dur*howm1))
###################################3
...
crn_1 = gp_signals.FourierBasisGP(spectrum = cpl_1, modes=freqs, name = 'gw')
crn_2 = gp_signals.FourierBasisGP(spectrum = cpl_2, modes=freqs, name = 'other_gw')
